...
validate_values() {
    ...
    if [ ${INTERVAL: -1} == 'm' ]; then
        INTERVAL="${INTERVAL::-1}"
    elif [ ${INTERVAL: -1} == 'h' ]; then
...
generate_binary_config() {
    ...
    if [[ "$current_md5" == "$breadcrumb_md5" ]]; then
        echo "Settings have not changed, so no need to regenerate the binary config file."
        return
    fi

    if ! expect "$EXPECT_SCRIPT" "$USERNAME" "$PASSWORD" "$DOMAINS" "$INTERVAL"; then
...
all_required_settings_exist() {
    ...
    for required_setting in $REQUIRED_SETTINGS; do
        if [ -z "$(eval "echo \$$required_setting")" ]; then
...
create_and_validate_config_file() {
    ...
    if diff "$TEMPLATE_CONFIG_FILE" "$CONFIG_FILE" >/dev/null; then
...
set_default_values() {
    # Handle defaults now
    for KEY_VALUE in $DEFAULT_SETTINGS; do
